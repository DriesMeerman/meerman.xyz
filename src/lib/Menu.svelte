<script>
  import { darkMode, toggleDarkMode, particlesEnabled, toggleParticles } from '$lib/state.svelte.js';
  ...
  let darkModeIconFill = $derived(darkMode.current ? 'white' : 'black');
  let particleText = $derived(particlesEnabled.current ? 'Disable particles' : 'Enable particles');
  ...
  <div class="w-full block flex-grow sm:flex sm:items-center sm:w-auto sm:h-7 transition-all duration-500 sm:overflow-visible overflow-hidden {expanded ? 'max-h-48' : 'max-h-0'}">
    ...
    <div class="flex flex-row mt-2 sm:mt-0 gap-4 sm:gap-2">
      <div class="hover:animate-pulse" title={particleText} role="button" tabindex="0" onclick={() => toggleParticles()} onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { toggleParticles(); e.preventDefault(); } }}>
        <svg class={getIconClass(darkMode.current)} width="24px" height="24px" viewBox="0 0 16 16">
      ...
      <div>
        <div tabindex="0" class="cursor-pointer mtop-0 sm:mtop-6" role="button" onclick={toggleDarkMode} onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { toggleDarkMode(); e.preventDefault(); } }} title={darkMode.current ? 'Lightmode' : 'Darkmode'}>
          <svg class={getIconClass(darkMode.current)} width="24px" height="24px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
